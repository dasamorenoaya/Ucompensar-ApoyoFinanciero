...
function App() {
  ...
  const uploadBrandAsset = async (file: File, kind: 'logo' | 'background') => {
    ...
    const maxSize = kind === 'logo' ? 700 * 1024 : 4 * 1024 * 1024;
  ...
  return (
    <div className="app-shell" style={themeStyle}>
      {settings.background_type === 'video' && settings.background_image_url && <video className="app-bg-video" src={settings.background_image_url} autoPlay muted loop playsInline aria-hidden="true" style={{ opacity: 'var(--bg-video-opacity)' }} />}
    ...
    <div className="overlay"><div className="modal settings-modal">
      ...
      <section className="settings-section">
        ...
        <div className="branding-grid">
          ...
          <div className="branding-card"><div className="branding-preview background">{s.background_type === 'video' && s.background_image_url ? <video src={s.background_image_url} autoPlay muted loop playsInline /> : s.background_image_url ? <img src={s.background_image_url} alt="Vista previa del fondo" /> : <b>Sin fondo</b>}</div><div className="branding-copy"><b>Fondo del banco</b><span>Imagen JPG/PNG o video corto MP4/WebM. El video se reproduce automáticamente, en silencio y en bucle. Imagen: máximo 700 KB · Video: máximo 4 MB.</span><label className="upload-logo-btn"><Upload size={16} /> Seleccionar fondo<input type="file" accept="image/jpeg,image/png,video/mp4,video/webm" onChange={e => { const file = e.target.files?.[0]; if (file) void chooseAsset(file, 'background'); e.currentTarget.value = ''; }} /></label>{s.background_image_url && <button className="link-danger" onClick={() => setS({ ...s, background_image_url: '', background_path: '', background_type: 'image', background_video_url: '', background_video_path: '' })}>Quitar fondo</button>}</div></div>
        </div>
      </section>
      <section className="settings-section">
